...
const taskReducer = (state, action) => {
    switch(action.type) {
        ...
        case TYPES.DELETE_TASK:
            return {
                ...state,
                completedTasks: state.completedTasks.filter(t => t.id !== action.deletedTask.id)
            }
        default:
            return state;
...
function Tasks() {
    ...
    //double arrow to avoid state update infinite loop
    const completeTask = completedTask => () => {
        dispatch({type: TYPES.COMPLETE_TASK, completedTask})
    }

    const deleteTask = task => () => {
        dispatch({type: TYPES.DELETE_TASK, deletedTask: task})
    }
